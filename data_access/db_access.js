...
const getCityAirport = (request, response) => {
    pool.query('select * from airport a, city c where a.city_id = c.id order by c.city_state;', (error, results) => {
        if (error) {
        throw error
        }
        response.status(200).json(results.rows)
...
const getPassengerAircraft = (request, response) => {
    pool.query('select p.first_name, p.last_name, a.aircraft_type, a.airline_name from passenger p, aircraft a, passenger_aircraft pa where pa.passenger_id = p.id and pa.aircraft_id = a.id order by p.last_name;', (error, results) => {
        if (error) {
        throw error
        }
        response.status(200).json(results.rows)
...
const getAirportPassenger = (request, response) => {
    pool.query('select p.first_name, p.last_name, a.airport_name, a.airport_code from passenger p, airport a, passenger_airport pa where pa.passenger_id = p.id and pa.airport_id = a.id order by p.last_name;', (error, results) => {
        if (error) {
        throw error
        }
        response.status(200).json(results.rows)
